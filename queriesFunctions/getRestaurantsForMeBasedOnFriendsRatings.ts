...
export async function getRestaurantsForMeBasedOnFriendsRatings(userId: string) {
    let dc = new DriverRemoteConnection(`wss://${uri}/gremlin`, {})
    const graph = new Graph()
    const g = graph.traversal().withRemote(dc)

    const data = await g.V(userId).out("friends").out("writes").order().by("rating", gremlin.process.order.desc).out("are_about").toList();
